...
agent  0        true   text   "Hi, you've reached the front desk. How can I help?"
       ...
       (the caller talks for a few seconds — nothing arrives)
...
- **The caller does not.** Nothing is sent while they talk. Their whole
  sentence arrives in one message once their turn is over, about a second
  after they stop.
- **The caller's message can arrive late.** It often lands just after the
  agent's reply has started, even though it belongs before it.
...
<ResponseField name="ordinal" type="integer" required>
  ...
  share ordinal `0`, so you know to join them. It is also the true order: the
  caller's ordinal `1` belongs before the agent's `2` even though it arrived
  after. Sort by it, not by arrival.
</ResponseField>
